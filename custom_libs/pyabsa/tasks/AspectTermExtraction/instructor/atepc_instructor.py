    ...
    def _train_and_evaluate(self, criterion):
        ...
        for epoch in range(int(self.config.num_epoch)):
            ...
            for step, batch in enumerate(iterator):
                ...
                iterator.refresh()
            self.logger.info(f'PROGRESS: Epoch:{epoch}|APC-ACC:{self.config.max_test_metrics["max_apc_test_acc"]}|APC-F1:{self.config.max_test_metrics["max_apc_test_f1"]}|ATE-F1:{self.config.max_test_metrics["max_ate_test_f1"]}')
            if patience == 0:
                break
