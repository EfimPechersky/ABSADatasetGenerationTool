...
class RNARTrainingInstructor(BaseTrainingInstructor):
    ...
    def _train_and_evaluate(self, criterion):
        ...
        self.config.metrics_of_this_checkpoint = {"mse": 0}
        self.config.max_test_metrics = {"max_test_mse": 0}
                ...
                if global_step % self.config.log_step == 0:
                    if self.test_dataloader and epoch >= self.config.evaluate_begin:
                        if self.valid_dataloader:
                            test_mse = self._evaluate_mse(
                                self.valid_dataloader, criterion
                            )
                        else:
                            test_mse = self._evaluate_mse(
                                self.test_dataloader, criterion
                            )

                        self.config.metrics_of_this_checkpoint["mse"] = test_mse

                        if test_mse < max_fold_mse:
                            if test_mse < max_fold_mse:
                                patience = self.config.patience - 1
                                max_fold_mse = test_mse
                            ...
                            if self.config.model_path_to_save:
                                ...
                                if save_path and self.config.save_last_ckpt_only:
                                    try:
                                        ...
                                        pass
                                save_path = "{0}/{1}_{2}_mse_{3}/".format(
                                    self.config.model_path_to_save,
                                    self.config.model_name,
                                    self.config.dataset_name,
                                    round(test_mse, 4),
                                ...
                                if (
                                    test_mse
                                    < self.config.max_test_metrics["max_test_mse"]
                                ):
                                    self.config.max_test_metrics["max_test_mse"] = (
                                        test_mse
                                    )
                        ...
                        description = "Epoch:{} | Loss:{:.4f} | Dev R2 Score:{:.4f}(max:{:.4f})".format(
                            epoch, loss.item(), test_mse, max_fold_mse
                        )
        ...
        if self.valid_dataloader:
            ...
            max_fold_mse = self._evaluate_mse(self.test_dataloader, criterion)
    ...
    def _k_fold_train_and_evaluate(self, criterion):
        ...
        for f, (train_dataloader, valid_dataloader) in enumerate(
            ...
            for epoch in range(self.config.num_epoch):
                ...
                for i_batch, sample_batched in enumerate(iterator):
                    ...
                    if global_step % self.config.log_step == 0:
                        if self.test_dataloader and epoch >= self.config.evaluate_begin:
                            test_mse = self._evaluate_mse(valid_dataloader, criterion)
            ...
            max_fold_mse = self._evaluate_mse(self.test_dataloader, criterion)
            if max_fold_mse < max_fold_mse_k_fold:
                save_path_k_fold = save_path
            fold_test_mse.append(max_fold_mse)
        ...
        max_test_mse = numpy.max(fold_test_mse)
    ...
    def _evaluate_mse(self, test_dataloader, criterion):
